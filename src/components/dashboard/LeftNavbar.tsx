...
import type { Menus } from '@/components/dashboard/layout/menu';

const LeftNavbar = (menu: Menus[]) => {
    ...
    <aside className=' absolute z-[1] h-screen w-64 bg-[#fdf6f1]/80'>
      <div className='mx-auto mb-10 flex items-center bg-[#D95B60] px-4 pt-6'>
        <Image
          ...
          width={100}
          height={100}
          className='mx-auto mb-6 mt-11 h-14 rounded-2xl'
        />
        ...
        <ul className='pt-2'>
          ...
          {menu.map((menu, index) => (
            <>
              ...
              <li
                key={index}
                className={`text-gray-800 mt-2 flex cursor-pointer items-center gap-x-4 border-b-[1px] border-slate-600  p-2 text-sm hover:bg-white ${
              ...
              {menu.submenu && submenuOpen[index] && (
                <ul className=''>
                  {menu.submenuItems?.map((submenuItems, index) => (
                    <li
                      key={index}
                      className='text-gray-800 mt-2 flex cursor-pointer items-center border-b-[1px] border-slate-600 p-2 px-10 text-sm hover:bg-white '
                      onClick={() =>
                        setsubmenuOpenlv2({
                          ...submenuOpenlv2,
